...
def open_file(path: str | Path, file_manager: bool = False, windows_start_command: bool = False):
    ...
    try:
        if sys.platform == "win32":
            ...
            if file_manager:
                # Spawn explorer directly; no cmd.exe in the chain means
                # shell metacharacters in the filename cannot be injected.
                silent_popen(
                ...
                # os.startfile -> ShellExecuteW; the filename is never parsed
                # as a command line, so '&', '^', etc. in filenames are safe.
                # Folds in the legacy 'start'-command path -- the behavior
                # split was a workaround for the shell route we just removed.
                os.startfile(normpath)  # noqa: S606
        else:
